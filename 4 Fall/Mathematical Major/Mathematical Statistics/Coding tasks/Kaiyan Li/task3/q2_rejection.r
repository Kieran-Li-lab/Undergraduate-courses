...
## ----- Q2: g(x) = 0.7*Unif[-1,2] + 0.3*(2+Exp(1)) -----


## ---------- Q2: “不那么合适”的 g，仅供拒绝采样 ----------
## g = 0.5*Arcsine[-1,1] + 0.5*PowerTail(x>=2),  PowerTail ~ (x-1)^(-3/2)
...
g_pdf_q2 <- function(x){
  ...
  # arcsine-like on [-1,1]
  idx1 <- (x > -1) & (x < 1)
  y[idx1] <- (3/8) * (1 - x[idx1]^2)
  # power tail on [2, inf)
  idx2 <- x >= 2
  y[idx2] <- 0.5 * exp(- (x[idx2] - 2))
  y
}
...
cat("=== Q2: Theoretical vs 3 methods ===\n")
